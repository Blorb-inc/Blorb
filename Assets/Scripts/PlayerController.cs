...
public class PlayerController : MonoBehaviour
{
    ...
    {
        //  ABILITY FOR SMALL SIZE

        if ((int)GameManager.Instance.playerSize == 0)
        {
            if (isDashing == false && Time.time > NextAbilityUse && (Input.GetKeyDown(KeyCode.Space) || Input.GetKeyDown(KeyCode.JoystickButton1))) // Dash Check
            {
                isDashing = true;
                speedFlame.SetActive(true);
                smokePoof.Play();
                AudioManager.instance.Play("Whoosh");
                dashStartTime = 0.4f;
                NextAbilityUse = Time.time + AbilityCoolDown;
                speedDash = 2500f;
            }
        }

        //  ABILITY FOR MEDIUM SIZE
        if ((int)GameManager.Instance.playerSize == 1)
        {
            if (isGrounded && !isJumping && (minJump > 0f || Input.GetKey(KeyCode.Space) || Input.GetKey(KeyCode.JoystickButton1))) // Jump down button check
            {
                if (!isJumping)
                {
                    curveEval = 0f;
        ...
        if ((int)GameManager.Instance.playerSize == 2)
        {
            if (isGrounded && !isJumping && (minJump > 0f || Input.GetKey(KeyCode.Space) || Input.GetKey(KeyCode.JoystickButton1))) // Jump down button check
            {
                ...
                isJumping = true;
                currentJumpPower = bigJumpPower;
            }
        }

        if (isJumping) // Jump is being held
        {
            curveEval += Time.deltaTime * 3f;
        }
        else
        {
            isJumping = false;
